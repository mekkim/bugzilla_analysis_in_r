...
#
# Current version: February 4, 2016
# 
# ---
...
# 5. A `PHP utility script` for domain name parsing
# 6. A `PERL` installation (tested on ActivePerl 5.22.1.2201)
# 7. A `Tree-Tagger` installation
# 
...
# 
# * Add the mysql/bin folder to the PATH system environment. Its default location is C:\Program Files\MySQL\MySQL Server 5.5\bin, but could
# vary depending on your install parameters
# 
# ---
# ## RESTORE BUGZILLA DATABASE (Current version includes data until end of 2012)
# 
# * Decompress/untar the Bugzilla database.  The result is a MySQL-formatted dumpfile with a .sql extension. I'll assume that the file name is "bugzilla.sql". If it's not, change the name in the commands below. 
# NOTE: This dumpfile only works with MySQl databases.  It cannot be restored to other databases such as SQLite, PostGRESQL, MSSQL, etc. It is also sufficiently complex that scripts cannot readily be used to convert it to a dumpfile of a different format
# 
...
# NOTE: This version uses PHP 5.6.14, which was current release version at the time. I chose x64 Thread Safe, but the other ones should work too.
# 
...
# NOTE: This script assumes you install Tree Tagger to its default of c:/TreeTagger.  If not, adjust accordingly in the user-defined parameters below
# NOTE: Pay particular attention to the PATH environment variable setting so that the R script can find the Tree Tagger script files
# 
